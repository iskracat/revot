...
from wtforms import (SelectMultipleField, SubmitField, StringField,
                     ...
                     ValidationError, FieldList, SelectField, PasswordField)
from wtforms.fields.html5 import IntegerField as StepIntegerField
from wtforms.widgets import CheckboxInput, ListWidget, HTMLString
...
class LoginForm(Form):
    ...
    user = StringField(lazy_gettext(u'Usuari'),
                       [Length(min=4, max=20), InputRequired()],
                       description=lazy_gettext(u'Username'))

    password = PasswordField(lazy_gettext(u'Password'),
                             [InputRequired()],
                             description=lazy_gettext(u'Password'))
    ...
    def validate_options(self, field):
        if not(self.min <= len(field.data) <= self.max):
            raise ValidationError(
                lazy_gettext(u'You must choose between %s and %s options') %
                            (self.min, self.max))
...
class DynamicListWidget(object):
    """
    Renders a list of fields that grows at user demand. Assumes simple
    subfields.
    """
...
class VotingForm(Form):
    ...
    title = StringField(lazy_gettext(u'Id'),
                        [Length(min=4, max=20), InputRequired()],
                        description=lazy_gettext(u'Public identifier used for the voting (max 20 chars).'))  # noqa

    language = SelectField(lazy_gettext(u'Language'),
                            description=lazy_gettext(u'Ballots will be written in this language.'))  # noqa

    descr = TextAreaField(lazy_gettext(u'Description'),
                          [InputRequired(), Length(max=100)],
                          description=lazy_gettext(u'A short description of the voting (max 100 chars).'))  # noqa

    roll = TextAreaField(lazy_gettext(u'Voting roll'),
                         [InputRequired(),
                          Regexp(ur'^(?:\w+(?:\.\w+)*@\w+(?:\.\w+)* *, *[ \.\-\w]+\s*)+$',  # noqa
                                 re.UNICODE,
                                 lazy_gettext(u'Wrong data format.'))],
                          description=lazy_gettext(u'List of <email>, <name> for people enrolled in this voting.'))  # noqa
    ...
    duration = SelectField(lazy_gettext(u'Voting duration'),
                           coerce=int,
                           description=lazy_gettext(u'The voting is open during this time.'))  # noqa

    send_ballots = SelectField(lazy_gettext(u'Send ballots'),
                               coerce=int,
                               description=lazy_gettext(u'Send ballots this time before opening the voting.'))  # noqa

    question = TextAreaField(lazy_gettext(u'Question'),
                             [InputRequired(), Length(min=10, max=100)],
                             description=lazy_gettext(u'The question printed in the ballot.'))  # noqa

    options = FieldList(StringField('Option'),
                        min_entries=2,
                        widget=DynamicListWidget(),
                        label=lazy_gettext(u'Options'),
                        description=lazy_gettext(u'During the voting, a voter must choose from these options.'))  # noqa
    ...
    max_options = StepIntegerField(lazy_gettext(u'Max. choices'),
                                   [InputRequired()],
                                   default=1,
                                   description=lazy_gettext(u'In a ballot, a voter can mark at most this number of options.'))  # noqa
    ...
    def validate_options(form, field):
        if any(opt == '' for opt in field.data):
            raise ValidationError(
                lazy_gettext(u'All option fields are required.'))
    ...
    def validate_send_ballots(form, field):
        ...
        if field.data < 0:
            ...
            min_to_send = current_app.config['MIN_TIME_TO_SEND_BALLOTS_IN_MINUTES']  # noqa
            print "Min to send ->", min_to_send
            if time_left < delta(minutes=min_to_send):
                raise ValidationError(lazy_gettext(u'Need to send ballots at least {0:d} minutes before voting begins.').format(min_to_send))  # noqa
        else:
            # Send ballots `field.data` minutes before: check if possible
            if time_left < delta(minutes=field.data):
                raise ValidationError(lazy_gettext(u'Not enough time before voting starts.'))  # noqa

    def validate_max_options(form, field):
        if field.data < 1 or field.data > len(form.options.data):
            raise ValidationError(lazy_gettext(u'Must be between 1 and the number of options.'))  # noqa
