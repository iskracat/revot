...
def activate_balloting(voting_id, when):
    return start_balloting_task.apply_async((voting_id,), eta=when+delta(seconds=2))
...
def start_balloting_task(voting_id):
    ...
    for voter in voting.voters:
        args = [voter.id, vtitle, vstarts, vends, bnmax, bnmin, bdesc, boptions, lang]
...
@celery.task
def send_mail_to_a_voter(voter_id, vtitle, vstarts, vends, bnmax, bnmin, bdesc, boptions, lang):
    ...
    # Replace email identity by random identity. Remove any traces of old identity.
    # get a random id and check it is unique
    randid = b64enc(urandom(RANDOM_ID_LENGTH))
    while db.session.query(Voter).\
          filter(Voter.identity == randid, Voting.id == voting.id).\
          count():
        randid = b64enc(urandom(RANDOM_ID_LENGTH))
    ...
    data['url']    =   url_for('main.do_vote', ident=voting.id, person=voter.identity, _external=True)
